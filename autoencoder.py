...
class Autoencoder(SavableModule):
    def __init__(self, is_variational = True):
        ...
        self.encoder = nn.Sequential(
            ...
            nn.Conv3d(in_channels = 4 * amcm, out_channels = 8 * amcm, kernel_size = 4, stride = 2, padding = 1),  # New layer
        ...
        self.decoder = nn.Sequential(            
            ...
            nn.ConvTranspose3d(in_channels = 8 * amcm, out_channels = 4 * amcm, kernel_size = 4, stride = 2, padding = 1),  # New layer
            nn.BatchNorm3d(4 * amcm),
            nn.LeakyReLU(negative_slope=0.2, inplace=True),
